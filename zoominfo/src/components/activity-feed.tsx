...
interface Props {
  initial: Array<AgentActionRecord>
}

export function ActivityFeed({ initial }: Props) {
  const [actions, setActions] = useState<Array<AgentActionRecord>>(initial)
  ...
  useEffect(() => {
    ...
    async function poll() {
      try {
        ...
        if (r.ok && !cancelled) {
          const data = (await r.json()) as { actions: Array<AgentActionRecord> }
  ...
  if (actions.length === 0) {
    return (
      <div className="px-6 py-12 text-center text-sm text-muted-foreground">
        No agent activity yet. Ask your agent to search ZoomInfo for contacts or companies.
      </div>
    )
  }
        ...
        {actions.map((a) => (
          ...
          >
            <div className="flex items-start justify-between gap-4">
              ...
              {a.zoominfo_deep_link && (
                <a
                  href={a.zoominfo_deep_link}
                  target="_blank"
