...
type ToolFn<A, T> = (
  args: A,
) => Promise<Result<T & ToolSuccessMeta, InstantlyError>>
...
export function wrapTool<A, T>(
  ...
  return async (args: A) => {
    ...
    let result: Result<T & ToolSuccessMeta, InstantlyError>
    try {
...
function recordAction<A, T>(
  toolName: string,
  args: A,
  result: Result<T & ToolSuccessMeta, InstantlyError>,
  duration: number,
): void {
  const db = getDb()
  const row: AgentActionRecord = {
    ...
    instantly_object: result.ok ? result.data.instantly_object ?? null : null,
    instantly_record_id: result.ok ? result.data.instantly_record_id ?? null : null,
    instantly_deep_link: result.ok ? result.data.instantly_deep_link ?? null : null,
