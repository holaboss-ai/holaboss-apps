...
import type { InstantlyError, Result } from "../lib/types"

// Instantly's REST surface is rooted at /api/v2 (v1 is being deprecated).
// Auth: Bearer <api_key> in Authorization header — handled by the broker.
...
function defaultClient(): BridgeLike {
  return createIntegrationClient("instantly") as BridgeLike
}
...
function isNotConnectedError(e: unknown): boolean {
  ...
  return (
    msg.includes("no instantly integration") ||
...
export async function call<T>(
  ...
  endpoint: string,
  body?: unknown,
): Promise<Result<T, InstantlyError>> {
  const client = getBridgeClient()
  let resp
  try {
    resp = await client.proxy<T>({
      method,
      endpoint: `${INSTANTLY_BASE}${endpoint}`,
      body,
    })
  } catch (e) {
    if (isNotConnectedError(e)) {
      return {
        ...
        error: {
          code: "not_connected",
          message: "Instantly is not connected for this workspace.",
        },
      }
    }
    return {
  ...
  if (resp.status === 401 || resp.status === 403) {
    return {
      ...
      error: {
        code: "not_connected",
        message: extractErrorMessage(resp.data) ?? "Instantly credential rejected.",
      },
    }
  }
  if (resp.status === 404) {
    return {
      ...
      error: {
        code: "not_found",
        message: extractErrorMessage(resp.data) ?? "Instantly resource not found.",
      },
    }
  }
  if (resp.status === 429) {
  ...
  if (resp.status === 422 || resp.status === 400) {
    return {
      ...
      error: {
        ...
        message: extractErrorMessage(resp.data) ?? `Instantly returned HTTP ${resp.status}.`,
      },
    }
  }
