    ...
    return (
      <div className="flex items-center justify-between border-b border-amber-500/40 bg-amber-500/10 px-6 py-2 text-sm">
        <span className="text-amber-700 dark:text-amber-400">
          Not connected. Open Holaboss to connect HubSpot.
        </span>
        <a
          href={frontendUrl}
  ...
  return (
    <div className="flex items-center justify-between border-b border-border bg-background px-6 py-2 text-sm">
      <span className="flex items-center gap-2 text-foreground">
        ...
        Connected to HubSpot{status.portal_id ? ` · portal ${status.portal_id}` : ""}
      </span>
      <a
        href={status.portal_id ? `https://app.hubspot.com/contacts/${status.portal_id}` : "https://app.hubspot.com"}
        target="_blank"
