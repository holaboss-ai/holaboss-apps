...
import type { ApolloError, Result, ToolSuccessMeta } from "../../src/lib/types"

describe("audit.wrapTool", () => {
  let tmp: string

  beforeEach(() => {
    tmp = mkdtempSync(path.join(tmpdir(), "apollo-audit-"))
  ...
  it("appends a success row for a successful call", async () => {
    const tool = wrapTool(
      ...
      async (
        _args: { foo: string },
      ): Promise<Result<{ apollo_record_id: string; result_summary: string } & ToolSuccessMeta, ApolloError>> => {
        return { ok: true, data: { apollo_record_id: "p_123", result_summary: "did a thing" } }
      },
    )
    ...
    expect(rows[0]).toMatchObject({
      tool_name: "apollo_test_tool",
      outcome: "success",
      apollo_record_id: "p_123",
  ...
  it("appends an error row for a failed call", async () => {
    const tool = wrapTool(
      "apollo_test_tool",
      async (): Promise<Result<{ apollo_record_id: string }, ApolloError>> => {
  ...
  it("clearActions truncates the table", async () => {
    const tool = wrapTool(
      "apollo_test_tool",
      async (): Promise<Result<Record<string, never>, ApolloError>> => {
        return { ok: true, data: {} }
      },
    )
    await tool({})
