...
export const fetchPosts = createServerFn({ method: "GET" }).handler(
  async () => {
    const db = getDb()
    return db
      .prepare("SELECT * FROM twitter_posts ORDER BY created_at DESC LIMIT 50")
      .all() as PostRecord[]
  },
)
...
export const updatePost = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const db = getDb()
    db.prepare(
      "UPDATE twitter_posts SET content = ?, scheduled_at = ?, status = 'draft', error_message = NULL, updated_at = datetime('now') WHERE id = ? AND status IN ('draft', 'failed')",
...
export const publishPost = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    db.prepare(
      "UPDATE twitter_posts SET status = ?, updated_at = datetime('now') WHERE id = ?",
...
export const cancelSchedule = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const db = getDb()
    db.prepare(
      "UPDATE twitter_jobs SET status = 'failed', error_message = 'Cancelled by user', updated_at = datetime('now') WHERE json_extract(payload, '$.post_id') = ? AND status IN ('waiting', 'delayed')",
    ).run(data.post_id)
    db.prepare(
      "UPDATE twitter_posts SET status = 'draft', scheduled_at = NULL, updated_at = datetime('now') WHERE id = ?",
    ).run(data.post_id)
    const post = db
      .prepare("SELECT * FROM twitter_posts WHERE id = ?")
      .get(data.post_id) as PostRecord
...
export const deletePost = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const db = getDb()
    const post = db
      .prepare("SELECT output_id FROM twitter_posts WHERE id = ?")
      .get(data.post_id) as Pick<PostRecord, "output_id"> | undefined
    db.prepare("DELETE FROM twitter_posts WHERE id = ?").run(data.post_id)
    if (post?.output_id) {
      try {
        await updateAppOutput(post.output_id, { status: "deleted" })
      } catch (syncError) {
        console.error(
          `[actions] twitter output mark-deleted failed for post ${data.post_id}:`,
          syncError,
        )
      }
    }
