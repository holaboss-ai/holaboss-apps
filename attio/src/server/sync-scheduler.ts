...
export function startSyncScheduler(): void {
  ...
  console.log("[sync] attio scheduler started (30min incremental, daily full)")
}
...
async function runSyncTick(): Promise<void> {
  ...
  try {
    ...
    const summary = result.per_object
      ...
      .join(" ")
    console.log(
      `[sync] attio kind=${dueFull ? "full" : "incremental"} ${summary}` +
        `${result.rate_limited ? " (rate-limited)" : ""}`,
    )
  } catch (err) {
    console.error("[sync] attio tick failed:", err)
  }
}
